...
void init_complementary_filter_vxy(ComplementaryFilter2_t *filter, float k1, float k2, float Ts){
    ...
    // G2(z^-1)
    filter->num2[0] = (2.0f*k1*Ts + k2*Ts*Ts)/tmp_den0;
    filter->num2[1] = (k2*Ts*Ts)/tmp_den0;
    filter->num2[2] = (-2.0f*k1*Ts + k2*Ts*Ts)/tmp_den0;
    ...
    filter->y[0] = 0.0f;
    filter->y[1] = 0.0f;
    DEBUG_PRINT("vxy numerator1: %f, %f, %f, numerator1: %f, %f, %f, denominator %f, %f, %f", 
        (double) filter->num1[0], (double) filter->num1[1], (double) filter->num1[2], 
        (double) filter->num2[0], (double) filter->num2[1], (double) filter->num2[2],
        (double) filter->den[0], (double) filter->den[1], (double) filter->den[2]);
}
...
void init_complementary_filter_z(ComplementaryFilter2_t *filter, float k1, float k2, float Ts){
    ...
    // G1(z^-1)
    filter->num1[0] = Ts*Ts/tmp_den0;
    filter->num1[1] = 2.0f*Ts*Ts/tmp_den0;
    filter->num1[2] = Ts*Ts/tmp_den0;
    ...
    filter->y[0] = 0.0f;
    filter->y[1] = 0.0f;
    DEBUG_PRINT("alt numerator1: %f, %f, %f, numerator1: %f, %f, %f, denominator %f, %f, %f", 
        (double) filter->num1[0], (double) filter->num1[1], (double) filter->num1[2], 
        (double) filter->num2[0], (double) filter->num2[1], (double) filter->num2[2],
        (double) filter->den[0], (double) filter->den[1], (double) filter->den[2]);
}
