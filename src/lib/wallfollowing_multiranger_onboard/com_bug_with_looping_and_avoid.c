/*
 * com_bug_with_looping_and_avoid.c
 *
...
int com_bug_loop_avoid_controller(float *vel_x, float *vel_y, float *vel_w, float front_range, float left_range,
                                  float right_range,
                                  float current_heading, float current_pos_x, float current_pos_y, uint8_t other_drone_rssi)
{
  ...
  //static float pos_x_switch = 0;
  //static float pos_y_switch = 0;

  static bool overwrite_and_reverse_direction = false;
  static float direction = -1;
  static bool cannot_go_to_goal = false;
  //static bool switch_wanted_angle = false;
  ...
  */
  ...
  if (state == 1) {     //FORWARD
    ...
    if ((state_wf == 6 || state_wf == 8) && goal_check_WF && front_range > ref_distance_from_wall + 0.4f
      ...
      if (other_drone_rssi < 58) {
        //pos_x_switch = 0;
        //pos_y_switch = 0;
        wanted_angle = -1 * wanted_angle;
        state = transition(4); //rotate_to_goal
  ...
  } else  if (state == 2 || state == 4) { //ROTATE_TO_GOAL
